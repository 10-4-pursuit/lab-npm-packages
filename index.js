...
function countClassesByInstructor(collection, instructorName) {
  let count = _.countBy(collection, 'instructor')
  if (!(count[instructorName])) {
    return "There is no instructor by that name."
  } else {
    return count[instructorName]
  } 
}
...
function getUniqueClasses(collection) {
  const uniq = _.uniqBy(collection, 'title')
  const output = uniq.map(yogaClass => {
    const title = yogaClass.title
    const priceInCents = yogaClass.priceInCents
...
function orderClassesByTitleAndLevel(collection) {
  const newCollection = _.orderBy(collection, ['title', 'level'], ['asc', 'desc'])
  return newCollection.map(({title, instructor,level}) => ({title, instructor, level}))
}
