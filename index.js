...
function orderClassesByTitleAndLevel(collection) {
  const groupByTitle = _.groupBy(collection, 'title');
  const sortedTitles = _.sortBy(Object.keys(groupByTitle));

  const result = [];

  sortedTitles.forEach(title => {
    const classesByTitle = groupByTitle[title];

    const groupByLevel = _.groupBy(classesByTitle, 'level');
    const sortedLevels = _.sortBy(Object.keys(groupByLevel)).reverse();

    sortedLevels.forEach(level => {
      groupByLevel[level].forEach(classObj => {
        result.push({
          title: title,
