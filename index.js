...
function countClassesByInstructor(collection, instructorName) {
 ...
 if (classesByInstructor.length > 0) {
  return classesByInstructor.length
 }else {
  ...
  // return _.filter((collection, instructorName));
...
return classes.map(({ title, priceInCents }) => ({ title, priceInCents }));
...
//   const classes = uniqueClasses.map()
 }
...
function orderClassesByTitleAndLevel(collection) {
  return _.orderBy(collection, ['title', 'level'], ['asc', 'desc']).map(({ title, instructor, level }) => ({ title, instructor, level }));
  // const orderedClasses = _.orderBy(collection, ['title', 'level'], ['asc', 'desc']);
  // const gym = orderedClasses.map(({ title, instructor, level }) => ({ title, instructor, level }));
  // return gym;
}
