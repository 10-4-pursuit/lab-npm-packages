...
function sumNumbers(array) {
  const numbers = _.remove(array, (n) => !!n)
...
function newMemberArrayToObject(member) {
 const newObject = _.fromPairs(member)
 return newObject
}
...
function omitAgeFromMembers(collection) {
  for(let member of collection){
    const removedKeys = _.unset(member, "age" )
  }
...
function countClassesByInstructor(collection, instructor) {
  const whoTaughtThis = _.countBy(collection, "instructor")
  return whoTaughtThis[instructor] || "There is no instructor by that name."  
...
function removeInactiveMembers(collection) {
  const inactiveMem = _.remove(collection, function(n) {
    return n.currentMember === false
  })
  return collection
}
...
function getUniqueClasses(collection) {
  const arrayForUniqueList = []
  const uniqueList = _.uniqBy(collection, function(a) {
    return `${a.title}, ${a.priceInCents}`
  })
  for(let item of uniqueList){
arrayForUniqueList.push({title: item.title, priceInCents: item.priceInCents})
  }

  return arrayForUniqueList
}
...
function orderClassesByTitleAndLevel(collection) {
  const organizedList = _.orderBy(collection, [`title`, `level`], [`asc`, `desc`])
  
  return organizedList.map(({title, instructor, level}) => ({title, instructor, level}))
}
